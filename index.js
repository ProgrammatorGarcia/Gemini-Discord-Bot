...
const FILE_PATHS = {
  ...
  kiyohimeAnger: path.join(CONFIG_DIR, 'kiyohime_anger.json')
};
...
//import { imageSearch } from './tools/function_calling.js'; // Make sure imageSearch is exported and imported
...
async function handleTextMessage(message) {
  ...
  // //<---- Kiyohime Anger Check ---->
  // const kiyohimeTriggerWords = ['lie', 'liar', 'deceive', 'stupid', 'hate you', 'lying', 'deceiving', 'betray'];
  // const ANGER_THRESHOLD = 3;
  // const containsTriggerWord = kiyohimeTriggerWords.some(word => message.content.toLowerCase().includes(word));

  // if (containsTriggerWord) {
  //   if (!kiyohimeAnger[userId]) {
  //     kiyohimeAnger[userId] = 0;
  //   }
  //   kiyohimeAnger[userId]++;
  //   console.log(`Kiyohime anger for user ${userId} is now ${kiyohimeAnger[userId]}`);
  // }
  // //<---- End of Check ---->
  ...
  let finalInstructions = isServerChatHistoryEnabled ? instructions + infoStr : instructions;

  // //<---- Kiyohime Noble Phantasm Trigger ---->
  // if (kiyohimeAnger[userId] >= ANGER_THRESHOLD) {
  //   finalInstructions += "\n\n[SYSTEM NOTE: The user has provoked you repeatedly. You are incredibly angry and feel betrayed. It is time to unleash your Noble Phantasm, Tenshin Kashō Zanmai. Declare it by name and let your rage flow.]";
  // }
  // //<---- End of Trigger ---->

  const ANGER_THRESHOLD = 3; // Keep the threshold definition here for clarity
  const currentAnger = kiyohimeAnger[userId] || 0;

  if (currentAnger > 0) {
    let systemNote = '';
    if (currentAnger >= ANGER_THRESHOLD) {
      // This is the final straw. The instruction is now a direct command to act.
      systemNote = `\n\n[SYSTEM NOTE: The user has provoked you to your absolute limit. Your anger level is ${currentAnger}. You feel utterly betrayed. It is time to unleash your Noble Phantasm, Tenshin Kashō Zanmai. You MUST declare it by name now and let your rage flow.]`;
      console.log(`Instructing Kiyohime to use NP for user ${userId}. Anger: ${currentAnger}`);
    } else {
      // The anger is building. The instruction guides the bot's tone.
      systemNote = `\n\n[SYSTEM NOTE: You are growing more and more angry with this user. They have lied to or insulted you ${currentAnger} time(s). Maintain your composure, but let your displeasure and suspicion subtly show in your tone.]`;
    }
    finalInstructions += systemNote;
  }
...
async function clearChatHistory(interaction) {
  try {
    ...
    delete kiyohimeAnger[interaction.user.id]; // Also reset anger level
    const embed = new EmbedBuilder()
      .setColor(0x00FF00)
...
async function handleWhitelistCommand(interaction) {
  try {
    ...
    if (index > -1) {
      ...
      delete kiyohimeAnger[userId]; // Reset anger level on whitelist
...
async function handleModelResponse(initialBotMessage, chat, parts, originalMessage, typingInterval, historyId) {
  ...
  while (attempts > 0 && !stopGeneration) {
    try {
      ...
      const newHistory = [];
      newHistory.push({ role: 'user', parts: parts }); // Note: role is 'user' and content is 'parts'
      async function getResponse(initialParts, message) {
        let partsForNextRequest = initialParts;
        let isDone = false;
        while (!isDone) {
          const result = await chat.sendMessageStream(partsForNextRequest);
          ...
          for await (const chunk of result.stream) {
            ...
            const imageSearchCall = chunk.functionCalls()?.find(tc => tc.name === 'image_search');
            if (imageSearchCall) {
              stopGeneration = true;
              clearTimeout(updateTimeout);
              if (botMessage) {
              ...
              const toolResult = await manageToolCall(imageSearchCall);
              const imageUrl = toolResult[0].parts[0].functionResponse.response.content;
              await message.channel.send(imageUrl);
              return 'IMAGE_SENT';
            }
            const toolCalls = chunk.functionCalls()?.filter(tc => tc.name !== 'image_search');

            if (toolCalls && toolCalls.length > 0) {
              foundToolCall = true;

              // Check if our specific tool was called to update the anger state.
              if (toolCalls.some(tc => tc.name === 'record_transgression')) {
                if (!kiyohimeAnger[userId]) {
                    kiyohimeAnger[userId] = 0;
                }
                kiyohimeAnger[userId]++;
                console.log(`Kiyohime anger for user ${userId} is now ${kiyohimeAnger[userId]} due to AI decision.`);
              }

              // Process ALL captured tool calls normally to complete the round trip.
              const functionCallParts = toolCalls.map(fc => ({ functionCall: fc }));
              newHistory.push({ role: 'model', parts: functionCallParts });
              const toolCallsResults = [];
              for (const toolCall of toolCalls) {
                const toolResult = await manageToolCall(toolCall);
                toolCallsResults.push(...toolResult);
              }
              functionCallsString = functionCallsString.trim() + '\n' + `- ${processFunctionCallsNames(toolCalls)}`;
              newHistory.push(...toolCallsResults);
              partsForNextRequest = newHistory.slice(-1)[0].parts; 
              break; // Exit inner loop to start the next request with the tool results.
            }
            ...
            if (chunkText) {
              ...
              if (finalResponse.length > maxCharacterLimit && !isLargeResponse) {
                ...
                botMessage.edit({ embeds: [new EmbedBuilder().setColor(0xFFFF00).setTitle('Response Overflow').setDescription('The response got too large...')] });
        ...
        if (finalResponse.trim()) {
            newHistory.push({ role: 'model', parts: [{ text: finalResponse }] });
        }
        return 'CONTINUE';
      }
      const status = await getResponse(parts, originalMessage);
      if (status === 'IMAGE_SENT') {
        break;
      }

      // <---- Kiyohime Noble Phantasm Logic ---->
      const finalResponseText = finalResponse.toLowerCase();
      if (finalResponseText.includes('tenshin kashō zanmai') || finalResponseText.includes('tenshin kashou zanmai') 
        || finalResponseText.includes('tenshin kasen zanmai')) {
        const guildId = originalMessage.guild?.id;
        const channelId = originalMessage.channel.id;

        // 1. Blacklist the user who triggered the Noble Phantasm.
        if (guildId) {
            initializeBlacklistForGuild(guildId);
            if (!blacklistedUsers[guildId].includes(userId)) {
                blacklistedUsers[guildId].push(userId);
            }
        }

        // 2. Determine the correct history ID based on server/channel settings.
        const isServerChatHistoryEnabled = guildId ? serverSettings[guildId]?.serverChatHistory : false;
        const isChannelChatHistoryEnabled = guildId ? channelWideChatHistory[channelId] : false;
        const historyIdToDelete = isChannelChatHistoryEnabled ? (isServerChatHistoryEnabled ? guildId : channelId) : userId;

        // 3. Delete the correctly identified chat history.
        if (chatHistories[historyIdToDelete]) {
            delete chatHistories[historyIdToDelete];
            console.log(`Kiyohime erased chat history for ID: ${historyIdToDelete}`);
        }

        // 4. Reset the user's anger level.
        if (kiyohimeAnger[userId]) {
            delete kiyohimeAnger[userId];
        }

        console.log(`User ${userId} has been blacklisted and history erased by Kiyohime's Noble Phantasm.`);
      }
      // <---- End of Logic ---->
...
function getHistory(id) {
  ...
  // This mapping logic is now robust and handles all cases correctly.
  return combinedHistory.map(entry => {
    ...
    switch (entry.role) {
      case 'user':
        role = 'user';
        break;
      case 'assistant': // Legacy support for old history
      case 'model':
        role = 'model';
        break;
      case 'function':
        ...
        break;
      default:
        // This case should ideally not be hit, but it prevents crashes.
        // We will filter out any entries that don't have a valid role.
        return null;
    }
    return {
      role: role,
  ...
  }).filter(Boolean); // This removes any null entries from the final array.
}
...
function updateChatHistory(id, newHistory, messagesId) {
  ...
  // The newHistory array is already correctly formatted, so we can just append it.
